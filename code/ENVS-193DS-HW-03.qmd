...
These two tests have led me to the same interpretation of results. In the Spearman correlation test, I found a moderate negative relationship (Spearmans $\rho$ = 0.689, S = 9216.1, p = 1.29e-05, $\alpha$ = 0.05) between temperature and kelp elongation. This information leads me to reject the null hypothesis that rho is equal to 0.
...
The test I am focusing on does not have a figure or a table, so below I have added a different table from the paper.
...
This table shows the macro invertebrate community indices in urban and natural streams in the Santa Monica Mountains and Simi Hills, CA in 2001. It displays rows such as taxonomic richness and percent EPT (aquatic insect orders Ephemeroptera, Plecoptera, and Trichoptera) invertebrates, and compares urban and natural streams. Data points marked with different superscripts identify significant differences between the two at different p-values, bonferroni comparison, and others. Once the reader looks at the legend at the bottom, the table clearly represents the statistical tests described in the paper. 

## (c) 

The table is very simple and easy to read (no visual clutter). However, one critique is that the most critical part for understanding the statistical tests is very small at the bottom. The choice of italicizing this information does help draw the reader's eye, but it is still difficult to read and comprehend in this formatting. 

## (d) 
One recommendation would be to make the legend at the bottom bold. This would serve the same purpose as the italics of drawing the reader's eye, but make it more prominent and easily readable without adding too much visual clutter.